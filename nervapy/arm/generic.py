...
class ArithmeticInstruction(Instruction):
    def __init__(self, name, destination, source_x, source_y, origin=None):
        ...
        if (
            ...
            and source_y.is_modified_immediate12()
        ):
            pass
        elif (
            destination.is_general_purpose_register()
            and source_x.is_general_purpose_register()
            and source_y.is_shifted_general_purpose_register()
        ):
            pass
        else:
            raise ValueError(
...
class CompareInstruction(Instruction):
    def __init__(self, name, source_x, source_y, origin=None):
        ...
        if name in allowed_instructions:
            super(CompareInstruction, self).__init__(
                name, [source_x, source_y], origin=origin
            )
        else:
            raise ValueError(
                "Instruction {0} is not one of the allowed instructions ({1})".format(
        ...
        if (
            source_x.is_general_purpose_register()
            and source_y.is_modified_immediate12()
        ):
            pass
        elif (
...
class MovInstruction(Instruction):
    def __init__(self, name, destination, source, origin=None):
        ...
        if name in allowed_instructions:
            super(MovInstruction, self).__init__(
                name, [destination, source], origin=origin
            )
        else:
            raise ValueError(
                "Instruction {0} is not one of the allowed instructions ({1})".format(
        ...
        if (
            destination.is_general_purpose_register()
            and source.is_modified_immediate12()
        ):
            pass
        elif (
            destination.is_general_purpose_register()
...
class LoadStoreInstruction(Instruction):
    ...
    def __init__(self, name, register, address, increment, origin=None):
        allowed_instructions = (
            LoadStoreInstruction.load_instructions
            + LoadStoreInstruction.store_instructions
        )
        if name not in allowed_instructions:
            raise ValueError(
                "Instruction {0} is not one of the allowed instructions ({1})".format(
        ...
        if (
            register.is_general_purpose_register()
            and address.is_memory_address(offset_bits=8)
            and increment.is_none()
        ):
            super(LoadStoreInstruction, self).__init__(
                name, [register, address], origin=origin
            )
        elif (
            name in {"STR", "LDR", "LDRB", "STRB", "STR.W", "LDR.W", "LDRB.W", "STRB.W"}
            and register.is_general_purpose_register()
            and address.is_memory_address(offset_bits=12)
            and increment.is_none()
        ):
            super(LoadStoreInstruction, self).__init__(
                name, [register, address], origin=origin
            ...
            and increment.is_offset8()
        ):
            super(LoadStoreInstruction, self).__init__(
                name, [register, address, increment], origin=origin
            )
        elif (
            register.is_general_purpose_register()
            and address.is_memory_address(offset_bits=0, allow_writeback=False)
            and increment.is_offset12()
        ):
            super(LoadStoreInstruction, self).__init__(
                name, [register, address, increment], origin=origin
            )
            ...
            if increment.is_none():
                ...
                raise ValueError(
                    "Invalid operands in instruction {0} {1}, {2}, {3}".format(
                        name, register, address, increment
                    )
                )
    ...
    def get_input_registers_list(self):
        input_registers_list = (
            self.operands[0].get_registers_list()
            if self.name in LoadStoreInstruction.store_instructions
            else list()
        )
    ...
    def get_output_registers_list(self):
        output_registers_list = (
            self.operands[0].get_registers_list()
            if self.name in LoadStoreInstruction.load_instructions
            else list()
        )
...
class PushPopInstruction(Instruction):
    def __init__(self, name, register_list, origin=None):
        allowed_instructions = {"PUSH", "POP", "PUSH.W", "POP.W"}
        if name in allowed_instructions:
            super(PushPopInstruction, self).__init__(
                name, [register_list], origin=origin
            )
        else:
            raise ValueError(
    ...
    def get_input_registers_list(self):
        if self.name in ("PUSH", "PUSH.W"):
            return self.operands[0].get_registers_list()
        else:
            return list()

    def get_output_registers_list(self):
        if self.name in ("POP", "POP.W"):
            return self.operands[0].get_registers_list()
        else:
            return list()


class StoreMultipleInstruction(Instruction):
    """Store/Load Multiple instructions (STMDB, LDMIA, etc.)"""

    def __init__(
        self, name, base_register, register_list, writeback=False, origin=None
    ):
        allowed_instructions = {
            "STMDB",
            "STMIA",
        ...
        if name in allowed_instructions:
            # Format: STMDB sp!, {r8, r9} or STMDB sp, {r8, r9}
            if writeback:
                base_operand = Operand(str(base_register) + "!")
            else:
                base_operand = Operand(base_register)
            super(StoreMultipleInstruction, self).__init__(
                name, [base_operand, register_list], origin=origin
            )
        ...
        if not register_list.is_general_purpose_register_list():
            raise ValueError(
                "Invalid register list in instruction {0} {1}, {2}".format(
                    name, base_register, register_list
                )
            )

    def get_input_registers_list(self):
        input_regs = []
        # Base register is always an input
        base_reg_name = str(self.operands[0]).rstrip("!")
        if base_reg_name in ("sp", "r13"):
            from nervapy.arm.registers import sp

            input_regs.append(sp)

        # For store instructions, register list is input
        if self.name.startswith("STM"):
            input_regs.extend(self.operands[1].get_registers_list())
    ...
    def get_output_registers_list(self):
        ...
        # If writeback, base register is output
        if self.writeback:
            base_reg_name = str(self.operands[0]).rstrip("!")
            if base_reg_name in ("sp", "r13"):
                from nervapy.arm.registers import sp

                output_regs.append(sp)

        # For load instructions, register list is output
        if self.name.startswith("LDM"):
            output_regs.extend(self.operands[1].get_registers_list())

        return output_regs

    def __str__(self):
        if self.writeback:
            return f"{self.name} {self.operands[0]}, {self.operands[1]}"
        else:
            return f"{self.name} {self.operands[0]}, {self.operands[1]}"
...
class BranchInstruction(Instruction):
    def __init__(self, name, destination, origin=None):
        ...
        if destination.is_label():
            ...
            raise ValueError(
                "Invalid operands in instruction {0} {1}".format("BX", destination)
            )
...
class BranchWithLinkInstruction(Instruction):
    def __init__(self, destination, origin=None):
        ...
        super(BranchWithLinkInstruction, self).__init__(
            "BL", [destination], origin=origin
        )
        self.lr = lr
        ...
        if destination.is_label():
            ...
            raise ValueError(
                "Invalid operands in instruction {0} {1}".format("BL", destination)
            )
...
class BranchExchangeInstruction(Instruction):
    def __init__(self, destination, origin=None):
        super(BranchExchangeInstruction, self).__init__(
            "BX", [destination], origin=origin
        )
...
class BranchLinkExchangeInstruction(Instruction):
    def __init__(self, destination, origin=None):
        ...
        super(BranchLinkExchangeInstruction, self).__init__(
            "BLX", [destination], origin=origin
        )
        self.lr = lr
        if destination.is_general_purpose_register():
            pass
            ...
            raise ValueError(
                "Invalid operands in instruction {0} {1}".format("BLX", destination)
            )
...
class DualRegisterLoadStoreInstruction(Instruction):
    ...
    def __init__(
        self, name, register_low, register_high, address, increment, origin=None
    ):
        allowed_instructions = (
            self.dual_load_instructions + self.dual_store_instructions
        )
        if name not in allowed_instructions:
            raise ValueError(
                "Instruction {0} is not one of the allowed instructions ({1})".format(
        ...
        if not (
            ...
            raise ValueError(
                "Both registers must be general-purpose registers in instruction {0}".format(
                    name
                )
            )
        ...
        if low_reg_id is not None and high_reg_id is not None:
            if low_reg_id % 2 != 0 or high_reg_id != low_reg_id + 1:
                raise ValueError(
                    "STRD/LDRD requires consecutive even/odd register pair: got r{0}, r{1}".format(
        ...
        if address.is_memory_address(offset_bits=8) and increment.is_none():
            ...
            super(DualRegisterLoadStoreInstruction, self).__init__(
                name, [register_low, register_high, address], origin=origin
            )
        elif (
            address.is_memory_address(offset_bits=0, allow_writeback=False)
            and increment.is_offset8()
        ):
            # Post-indexed addressing
            super(DualRegisterLoadStoreInstruction, self).__init__(
                name, [register_low, register_high, address, increment], origin=origin
            )
        else:
            if increment.is_none():
                raise ValueError(
                    "Invalid memory address in instruction {0}".format(name)
                )
            else:
                raise ValueError(
                    "Invalid operands in instruction {0}: address={1}, increment={2}".format(
    ...
    def get_output_registers_list(self):
        ...
        if (
            self.operands[2].is_writeback_memory_address()
            or self.operands[2].is_preindexed_memory_address()
        ):
...
class SGInstruction(Instruction):
    ...
    def __init__(self, origin=None):
        ...
        if (
            active_function
            and Extension.TrustZone not in active_function.target.extensions
        ):
...
class BranchExchangeNonSecureInstruction(Instruction):
    ...
    def __init__(self, destination, origin=None):
        ...
        super(BranchExchangeNonSecureInstruction, self).__init__(
            "BXNS", [destination], origin=origin
        )
        if not destination.is_general_purpose_register():
            raise ValueError(
                "Invalid operands in instruction BXNS {0}".format(destination)
            )
        if (
            active_function
            and Extension.TrustZone not in active_function.target.extensions
...
class BranchLinkExchangeNonSecureInstruction(Instruction):
    ...
    def __init__(self, destination, origin=None):
        ...
        super(BranchLinkExchangeNonSecureInstruction, self).__init__(
            "BLXNS", [destination], origin=origin
        )
        self.lr = lr
        if not destination.is_general_purpose_register():
            raise ValueError(
                "Invalid operands in instruction BLXNS {0}".format(destination)
            )
        if (
            active_function
            and Extension.TrustZone not in active_function.target.extensions
            ...
            raise ValueError(
                "BLXNS requires TrustZone extension (ARMv8-M architecture)"
            )
...
class TestTargetInstruction(Instruction):
    ...
    def __init__(self, name, destination, source, origin=None):
        ...
        if name not in self.allowed_names:
            raise ValueError(
                "Instruction {0} is not valid; expected one of: {1}".format(
                    name, ", ".join(self.allowed_names)
                )
            )
        super(TestTargetInstruction, self).__init__(
            name, [destination, source], origin=origin
        )
        if not destination.is_general_purpose_register():
        ...
        if (
            ...
            raise ValueError(
                "{0} requires TrustZone extension (ARMv8-M architecture)".format(name)
            )
...
def ADD(destination, source_x, source_y=None):
    ...
    instruction = ArithmeticInstruction(
        "ADD", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def ADC(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "ADC", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def SUB(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "SUB", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def SBC(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "SBC", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def RSB(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "RSB", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def RSC(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "RSC", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def AND(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "AND", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def BIC(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "BIC", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def ORR(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "ORR", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def EOR(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ArithmeticInstruction(
        "EOR", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LSL(destination, source_x, source_y=None):
    ...
    instruction = ShiftInstruction(
        "LSL", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LSR(destination, source_x, source_y=None):
    ...
    if source_y is None:
        destination, source_x, source_y = (destination, destination, source_x)
    instruction = ShiftInstruction(
        "LSR", Operand(destination), Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMP(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMP", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPEQ(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPEQ", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPNE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPNE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPCS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPCS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPHS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPHS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPCC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPCC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPLO(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPLO", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPMI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPMI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPPL(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPPL", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPVS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPVS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPVC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPVC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPHI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPHI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPLS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPLS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPGE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPGE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPLT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPLT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPGT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPGT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMPLE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMPLE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMN(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMN", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNEQ(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNEQ", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNNE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNNE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNCS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNCS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNHS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNHS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNCC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNCC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNLO(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNLO", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNMI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNMI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNPL(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNPL", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNVS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNVS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNVC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNVC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNHI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNHI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNLS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNLS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNGE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNGE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNLT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNLT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNGT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNGT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def CMNLE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "CMNLE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TST(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TST", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTEQ(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTEQ", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTNE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTNE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTCS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTCS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTHS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTHS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTCC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTCC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTLO(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTLO", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTMI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTMI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTPL(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTPL", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTVS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTVS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTVC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTVC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTHI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTHI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTLS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTLS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTGE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTGE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTLT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTLT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTGT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTGT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TSTLE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TSTLE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQ(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQ", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQEQ(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQEQ", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQNE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQNE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQCS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQCS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQHS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQHS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQCC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQCC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQLO(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQLO", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQMI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQMI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQPL(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQPL", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQVS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQVS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQVC(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQVC", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQHI(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQHI", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQLS(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQLS", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQGE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQGE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQLT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQLT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQGT(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQGT", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TEQLE(source_x, source_y):
    ...
    instruction = CompareInstruction(
        "TEQLE", Operand(source_x), Operand(source_y), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOV(destination, source):
    ...
    instruction = MovInstruction(
        "MOV", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVEQ(destination, source):
    ...
    instruction = MovInstruction(
        "MOVEQ", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVNE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVNE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVCS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVCS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVHS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVHS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVCC(destination, source):
    ...
    instruction = MovInstruction(
        "MOVCC", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVLO(destination, source):
    ...
    instruction = MovInstruction(
        "MOVLO", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVMI(destination, source):
    ...
    instruction = MovInstruction(
        "MOVMI", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVPL(destination, source):
    ...
    instruction = MovInstruction(
        "MOVPL", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVVS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVVS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVVC(destination, source):
    ...
    instruction = MovInstruction(
        "MOVVC", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVHI(destination, source):
    ...
    instruction = MovInstruction(
        "MOVHI", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVLS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVLS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVGE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVGE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVLT(destination, source):
    ...
    instruction = MovInstruction(
        "MOVLT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVGT(destination, source):
    ...
    instruction = MovInstruction(
        "MOVGT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVLE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVLE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSEQ(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSEQ", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSNE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSNE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSCS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSCS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSHS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSHS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSCC(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSCC", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSLO(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSLO", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSMI(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSMI", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSPL(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSPL", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSVS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSVS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSVC(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSVC", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSHI(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSHI", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSLS(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSLS", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSGE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSGE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSLT(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSLT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSGT(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSGT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVSLE(destination, source):
    ...
    instruction = MovInstruction(
        "MOVSLE", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MVN(destination, source):
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = MovInstruction(
        "MVN", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVW(destination, source):
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = MovInstruction(
        "MOVW", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def MOVT(destination, source):
    ...
    instruction = MovInstruction(
        "MOVT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def PUSH_W(register_list):
    """PUSH.W - 32-bit wide encoding for high registers in ARMv7-M"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = PushPopInstruction("PUSH.W", Operand(register_list), origin=origin)
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction


def POP_W(register_list):
    """POP.W - 32-bit wide encoding for high registers in ARMv7-M"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = PushPopInstruction("POP.W", Operand(register_list), origin=origin)
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction


def STMDB(base_register, register_list, writeback=True):
    """Store Multiple Decrement Before - equivalent to PUSH for stack operations"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = StoreMultipleInstruction(
        "STMDB", base_register, Operand(register_list), writeback, origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDMIA(base_register, register_list, writeback=True):
    """Load Multiple Increment After - equivalent to POP for stack operations"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = StoreMultipleInstruction(
        "LDMIA", base_register, Operand(register_list), writeback, origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STMIA(base_register, register_list, writeback=False):
    """Store Multiple Increment After"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = StoreMultipleInstruction(
        "STMIA", base_register, Operand(register_list), writeback, origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDMDB(base_register, register_list, writeback=False):
    """Load Multiple Decrement Before"""
    origin = (
        inspect.stack() if nervapy.arm.function.active_function.collect_origin else None
    )
    instruction = StoreMultipleInstruction(
        "LDMDB", base_register, Operand(register_list), writeback, origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STR(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STR", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRH(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRH", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRSH(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRSH", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STRH(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STRH", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRB(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRB", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRSB(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRSB", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STRB(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STRB", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRB_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRB.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STRB_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STRB.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDR_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDR.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STR_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STR.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def LDRH_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "LDRH.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STRH_W(register, address, increment=None):
    ...
    instruction = LoadStoreInstruction(
        "STRH.W", Operand(register), Operand(address), Operand(increment), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def STRD(register_low, register_high, address, increment=None, writeback=False):
    ...
    instruction = DualRegisterLoadStoreInstruction(
        "STRD",
        Operand(register_low),
        Operand(register_high),
        Operand(processed_address),
        Operand(increment),
        origin=origin,
    )
    if nervapy.stream.active_stream is not None:
...
def BXNS(destination):
    ...
    instruction = BranchExchangeNonSecureInstruction(
...
def BLXNS(destination):
    ...
    instruction = BranchLinkExchangeNonSecureInstruction(
        Operand(destination), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
...
def TT(destination, source):
    ...
    instruction = TestTargetInstruction(
        "TT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TTA(destination, source):
    ...
    instruction = TestTargetInstruction(
        "TTA", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
...
def TTT(destination, source):
    ...
    instruction = TestTargetInstruction(
        "TTT", Operand(destination), Operand(source), origin=origin
    )
    if nervapy.stream.active_stream is not None:
        nervapy.stream.active_stream.add_instruction(instruction)
    return instruction
